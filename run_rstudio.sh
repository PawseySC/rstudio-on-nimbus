...
NOTE: To re-run the rserver command, kill the current process first on your instance, then re-run the Ansible playbook. Kill the process by running: kill $(ps | grep 'rserver' | awk '{print \$1}')

IMPORTANT: After exiting RStudio, close the 8787 port on your local computer****: lsof -ti:8787 | xargs kill -9
-------------------------------------------------------------------------------------------'
